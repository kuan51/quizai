...
All commands run from root directory using Bun:
...
- **Database**: PostgreSQL via @vercel/postgres + Drizzle ORM. Schema in `src/lib/db/schema.ts`, instance in `src/lib/db/index.ts`. Migrations in `src/lib/db/migrations/`.
...
- Absolute imports via `@/` path alias (maps to `src/` directory)
